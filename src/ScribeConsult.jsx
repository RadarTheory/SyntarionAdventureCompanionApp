...
async function callGroq(systemPrompt, messages) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data?.error?.message || `Groq request failed with ${res.status}`);
  }
...
export function ScribeConsult({ char, onUpdateChar }) {
  ...
  const deductAP = async () => {
    ...
    if (error) {
      console.error('AP deduction failed:', error);
      throw new Error('Could not deduct AP.');
    }
...
This response costs the adventurer 1 AP. Make the answer useful, specific, and worthy of the cost.
`.trim();
      ...
      await deductAP();
      await persistScribeConsult({ question: userMsg, answer });
      ...
      setMessages(prev => [
        ...
        {
          role: 'scribe',
          content: `The archives resisted the inquiry. ${err?.message || 'Try again.'}`,
          time: new Date(),
        },
      ]);
    } finally {
  ...
  return (
    <>
      <button
        ...
        style={{
          width: '100%',
          background: open ? COLORS.deityBg : 'transparent',
        ...
        <div style={{ textAlign: 'left' }}>
          <div
            style={{
              ...
              fontSize: 11,
              fontWeight: 700,
              color: open ? COLORS.deityText : COLORS.text,
      ...
      {open && (
        <div
          style={{
            ...
            background: '#13100d',
            border: `1px solid ${COLORS.deity}55`,
            borderRadius: 14,
        ...
        >
          <div
            style={{
              padding: '12px 16px',
              borderBottom: `1px solid rgba(240,238,235,0.08)`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'space-between',
          ...
          {!ended ? (
            <div
              style={{
                padding: '10px 12px',
                borderTop: `1px solid rgba(240,238,235,0.08)`,
                display: 'flex',
                gap: 8,
              }}
            >
