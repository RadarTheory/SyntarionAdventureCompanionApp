...
export default function StepBackstory({
  ...
  // Pre-fill backstory from boon lines if empty
  useEffect(() => {
    if (!backstory && (boonOrigin || boonRole)) {
      const lines = [boonOrigin, boonRole].filter(Boolean).join('. ');
      setBackstory(lines + '.');
    }
  }, []);

  // Complication pool based on race
  ...
  if (submitted) {
    return (
      <div style={{
        ...
        }}>Awaiting the Scribe's Approval</div>
        <div style={{
          fontSize: 12,
          color: COLORS.muted,
          ...
          Your sheet remains visible in the meantime.
        </div>
        <div style={{
          fontSize: 10,
          color: COLORS.dim,
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        The boon is fixed — it was earned by your choices. The story is yours to write.
      </div>
  ...
  <button
    onClick={() => {
      const raceKey = race || 'default';
      const origins = BS_ORIGINS[raceKey] || BS_ORIGINS.default;
      const roles   = cp === 'magic' ? BS_ROLE.magic : cp === 'tech' ? BS_ROLE.tech : BS_ROLE.any;
      const newOrigin = pick(origins);
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{
            ...
            <div style={{
              ...
              color: COLORS.muted,
              fontFamily: "'Cinzel', serif",
            }}>Campaign hook (optional)</div>
            <button
              onClick={rerollComplication}
              style={{
                background: 'transparent',
                border: `1px solid ${COLORS.border}`,
                borderRadius: 4,
