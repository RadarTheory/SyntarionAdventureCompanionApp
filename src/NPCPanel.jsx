...
//  Data source: Supabase (npc_cities, npcs, npc_groups tables)
// ═══════════════════════════════════════════════════════════════════════════
...
const S = {
  ...
  modal: { position: 'absolute', inset: 0, background: 'rgba(8,6,4,0.96)', display: 'flex', flexDirection: 'column', zIndex: 20, padding: '14px', overflowY: 'auto' },
  modalTitle: { fontFamily: "'Cinzel', serif", fontSize: 12, color: '#e8c040', letterSpacing: '0.12em', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
...
const EMPTY_FORM = { name: '', role: '', status: 'Active', category: 'Uncategorized', group_name: '', faction: '', notes: '', city_id: 'unassigned' };

export default function NPCPanel({ campaignId, sessionId }) {
  const [cities, setCities]         = useState([]);
  const [groups, setGroups]         = useState([]); // flat array of all groups
  const [npcs, setNpcs]             = useState([]); // flat array of all npcs
  const [loading, setLoading]       = useState(true);
  const [collapsed, setCollapsed]   = useState({});
  const [catCollapsed, setCatCollapsed] = useState({});
  const [grpCollapsed, setGrpCollapsed] = useState({});
  const [search, setSearch]         = useState('');
  const [selected, setSelected]     = useState(null); // { npcId }
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const [{ data: cityData }, { data: npcData }, { data: groupData }] = await Promise.all([
      supabase.from('npc_cities').select('*').order('name'),
      supabase.from('npcs').select('*').order('name'),
      supabase.from('npc_groups').select('*').order('name'),
    ]);
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent('census:npc_snapshot', {
      detail: { npcs: npcs.map(n => ({ id: n.id, name: n.name, meta: [n.role, cities.find(c => c.id === n.city_id)?.name].filter(Boolean).join(' · '), conditions: n.conditions || [] })) },
    }));
  }, [npcs, cities]);

  useEffect(() => {
    const h = () => window.dispatchEvent(new CustomEvent('census:npc_snapshot', { detail: { npcs: npcs.map(n => ({ id: n.id, name: n.name, conditions: n.conditions || [] })) } }));
  ...
  useEffect(() => {
    const h = e => {
      ...
      if (!id) return;
      setNpcs(prev => prev.map(n => n.id === id ? { ...n, conditions } : n));
      supabase.from('npcs').update({ conditions }).eq('id', id);
    };
  ...
  useEffect(() => {
    const h = e => { const s = e.detail; if (!s?.name) return; setSuggestions(p => [...p, { ...s, _id: `${Date.now()}_${Math.random()}` }]); };
  ...
  useEffect(() => {
    ...
    supabase.from('characters').select('id, name, race, data, campaign_id').order('name').then(({ data }) => {
      if (!data) return;
      setPlayers(data.map(c => {
        const d = typeof c.data === 'string' ? JSON.parse(c.data || '{}') : (c.data || {});
        return { id: String(c.id), name: c.name || 'Unnamed', race: c.race || '', conditions: Array.isArray(d.conditions) ? d.conditions : [] };
      }));
    });
  }, [activeTab]);
  ...
  // ── NPC CRUD ──────────────────────────────────────────────────────────────
  const addNpc = async () => {
    const { name, role, status, category, group_name, faction, notes, city_id } = modalForm;
    if (!name.trim()) return;
    setSaving(true);
    const id = `npc_${newId()}`;
    const { data } = await supabase.from('npcs').insert({
      id, city_id, name: name.trim(), role, status,
      category: category || 'Uncategorized',
      group_name: group_name || '',
      faction, notes, conditions: [],
    }).select().single();
  ...
  const updateNpcField = async (npcId, field, value) => {
    setNpcs(prev => prev.map(n => n.id === npcId ? { ...n, [field]: value } : n));
    await supabase.from('npcs').update({ [field]: value }).eq('id', npcId);
  };

  const toggleNpcCondition = async (npcId, cond) => {
    const npc = npcs.find(n => n.id === npcId);
    if (!npc) return;
    const cur = npc.conditions || [];
    const next = cur.includes(cond) ? cur.filter(c => c !== cond) : [...cur, cond];
    setNpcs(prev => prev.map(n => n.id === npcId ? { ...n, conditions: next } : n));
    await supabase.from('npcs').update({ conditions: next }).eq('id', npcId);
  };
  ...
  const addCity = async (name) => {
    ...
    const { data } = await supabase.from('npc_cities').insert({ id, name: name.trim(), region: '' }).select().single();
    if (data) setCities(prev => [...prev, data]);
  };

  // ── Group CRUD ────────────────────────────────────────────────────────────
  const addGroup = async () => {
    const { city_id, category, name } = newGroupForm;
    if (!name.trim()) return;
    const id = `grp_${newId()}`;
    const { data } = await supabase.from('npc_groups').insert({ id, city_id, name: name.trim(), category }).select().single();
  ...
  // ── Player conditions ─────────────────────────────────────────────────────
  const togglePlayerCondition = async (playerId, cond) => {
    const player = players.find(p => p.id === playerId);
    if (!player) return;
    const cur = player.conditions || [];
    const next = cur.includes(cond) ? cur.filter(c => c !== cond) : [...cur, cond];
    setPlayers(prev => prev.map(p => p.id === playerId ? { ...p, conditions: next } : p));
    ...
    await supabase.from('characters').update({ data: { ...existing, conditions: next } }).eq('id', playerId);
  };
  ...
  const markMet = async (npc, cityName) => {
    ...
    await logSessionEvent(campaignId, sessionId, 'npc_met', {
      npc_name: npc.name, npc_role: npc.role || '', city: cityName,
  ...
  const acceptSuggestion = s => {
    ...
    setModalForm({ ...EMPTY_FORM, name: s.name || '', role: s.role || '', category: s.category || 'Uncategorized', faction: s.faction || '', notes: s.notes || '', city_id: matchCity ? matchCity.id : 'unassigned' });
    setModalMode('npc');
    setShowModal(true);
  ...
  const citiesWithNpcs = useMemo(() => {
    return cities.map(city => {
      let cityNpcs = npcs.filter(n => n.city_id === city.id);
      if (q) {
        const cityMatch = city.name.toLowerCase().includes(q) || (city.region || '').toLowerCase().includes(q);
        if (!cityMatch) cityNpcs = cityNpcs.filter(n =>
          ...
          (n.group_name || '').toLowerCase().includes(q) || (n.notes || '').toLowerCase().includes(q)
        );
      }
      return { ...city, npcs: cityNpcs, groups: groups.filter(g => g.city_id === city.id) };
    }).filter(city => !q || city.npcs.length > 0);
  }, [cities, npcs, groups, q]);

  const groupByCategory = (npcList) => {
    const map = {};
    npcList.forEach(n => { const cat = n.category || 'Uncategorized'; if (!map[cat]) map[cat] = []; map[cat].push(n); });
  ...
  const groupNpcsByGroup = (npcList) => {
    const g = {};
    npcList.forEach(n => { const grp = n.group_name || ''; if (!g[grp]) g[grp] = []; g[grp].push(n); });
    return g;
  };
  ...
  const modalCity = cities.find(c => c.id === modalForm.city_id);
  const modalGroups = modalCity ? groups.filter(g => g.city_id === modalCity.id && g.category === modalForm.category) : [];

  // ── NPC Row buttons ───────────────────────────────────────────────────────
  const NpcButtons = ({ npc, cityName }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
      <button onClick={e => { e.stopPropagation(); markMet(npc, cityName); }}
        style={{ background: 'rgba(184,137,42,0.12)', border: '1px solid rgba(184,137,42,0.3)', borderRadius: 3, color: '#e8c040', cursor: 'pointer', padding: '2px 6px', fontSize: 9, fontFamily: 'Georgia, serif' }}>⬡</button>
      <button onClick={e => { e.stopPropagation(); window.dispatchEvent(new CustomEvent('hercules:add_npc', { detail: { id: npc.id, name: npc.name, role: npc.role || '', conditions: npc.conditions || [], cityName } })); }}
        style={{ background: 'rgba(60,120,200,0.12)', border: '1px solid rgba(60,120,200,0.3)', borderRadius: 3, color: '#80a0e0', cursor: 'pointer', padding: '2px 6px', fontSize: 9, fontFamily: 'Georgia, serif' }}>⚔</button>
  ...
  if (loading) return (
    <div style={{ ...S.root, alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontFamily: "'Cinzel', serif", fontSize: 10, color: 'rgba(184,137,42,0.5)', letterSpacing: '0.14em' }}>Consulting the archives…</div>
    </div>
  );
  ...
  return (
    <div style={S.root}>
        ...
        <div style={S.modal}>
            ...
            <>
              ...
              <div style={S.field}>
                ...
                <input ref={modalNameRef} style={S.fieldInput} value={modalForm.name}
                  ...
                  placeholder="NPC name…" onKeyDown={e => e.key === 'Enter' && addNpc()} />
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ ...S.field, flex: 2 }}>
                  <label style={S.label}>Role / Title</label>
                  <input style={S.fieldInput} value={modalForm.role} onChange={e => setModalForm(f => ({ ...f, role: e.target.value }))} placeholder="Role or title…" />
                </div>
                <div style={{ ...S.field, flex: 1 }}>
                  <label style={S.label}>Status</label>
                  <select style={S.fieldSelect} value={modalForm.status} onChange={e => setModalForm(f => ({ ...f, status: e.target.value }))}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ ...S.field, flex: 1 }}>
                  ...
                  <select style={S.fieldSelect} value={modalForm.city_id} onChange={e => setModalForm(f => ({ ...f, city_id: e.target.value, group_name: '' }))}>
                    {cities.map(c => <option key={c.id} value={c.id} style={{ background: '#0e0c09' }}>{c.name}</option>)}
                  </select>
                </div>
                <div style={{ ...S.field, flex: 1 }}>
                  <label style={S.label}>Category</label>
                  <select style={S.fieldSelect} value={modalForm.category} onChange={e => setModalForm(f => ({ ...f, category: e.target.value, group_name: '' }))}>
              ...
              {modalGroups.length > 0 && (
                <div style={S.field}>
                  ...
                  <select style={S.fieldSelect} value={modalForm.group_name} onChange={e => setModalForm(f => ({ ...f, group_name: e.target.value }))}>
              ...
              <div style={S.field}>
                ...
                <input style={S.fieldInput} value={modalForm.faction} onChange={e => setModalForm(f => ({ ...f, faction: e.target.value }))} placeholder="Faction or affiliation…" />
              </div>
              <div style={S.field}>
                <label style={S.label}>Notes</label>
                <textarea style={{ ...S.fieldTextarea, minHeight: 56 }} value={modalForm.notes} onChange={e => setModalForm(f => ({ ...f, notes: e.target.value }))} placeholder="DM notes…" />
              </div>
              <div style={S.modalBtnRow}>
                <button style={S.modalConfirm} onClick={addNpc} disabled={saving}>{saving ? 'Adding…' : '✓ Add NPC'}</button>
          ...
          {modalMode === 'group' && (
            <>
              ...
              <div style={{ fontSize: 11, color: 'rgba(200,180,130,0.45)', fontStyle: 'italic', marginBottom: 10 }}>Groups appear as sub-folders inside a category.</div>
              <div style={S.field}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ ...S.field, flex: 1 }}>
                  <label style={S.label}>City</label>
                  <select style={S.fieldSelect} value={newGroupForm.city_id} onChange={e => setNewGroupForm(f => ({ ...f, city_id: e.target.value }))}>
                    {cities.map(c => <option key={c.id} value={c.id} style={{ background: '#0e0c09' }}>{c.name}</option>)}
                  </select>
                </div>
                <div style={{ ...S.field, flex: 1 }}>
                  <label style={S.label}>Category</label>
                  <select style={S.fieldSelect} value={newGroupForm.category} onChange={e => setNewGroupForm(f => ({ ...f, category: e.target.value }))}>
                    {CATEGORY_OPTIONS.map(c => <option key={c} value={c} style={{ background: '#0e0c09' }}>{c}</option>)}
                  </select>
                </div>
              </div>
      ...
      <div style={S.toolbar}>
        ...
        <button style={S.btnGold} onClick={() => { setModalForm(EMPTY_FORM); setModalMode('npc'); setShowModal(true); }}>+ NPC</button>
        <button style={S.btnGrey} onClick={() => { setNewGroupForm({ city_id: 'unassigned', category: 'Uncategorized', name: '' }); setModalMode('group'); setShowModal(true); }}>+ Group</button>
        <button style={S.btnGrey} onClick={() => { setModalForm({ ...EMPTY_FORM, _cityName: '' }); setModalMode('city'); setShowModal(true); }}>+ City</button>
      </div>
                  ...
                  {!isCollapsed && (
                    <>
                      ...
                      {catGroups.map(([cat, catNpcs]) => {
                        ...
                        const cityGroupsForCat = city.groups.filter(g => g.category === cat);
                        const allGroupNames = [...new Set([...cityGroupsForCat.map(g => g.name), ...Object.keys(byGroup).filter(k => k !== '')])];
                        ...
                        return (
                          <div key={cat} style={S.catBlock}>
                            <div style={S.catHeader} onClick={() => setCatCollapsed(c => ({ ...c, [catKey]: !c[catKey] }))}>
                              ...
                              <span style={S.catCount}>{catNpcs.length}</span>
                            </div>
                            ...
                            {!isCatColl && (
                              <>
                                {allGroupNames.map(grpName => {
                                  ...
                                  return (
                                    <div key={grpName} style={S.groupBlock}>
                                      <div style={S.groupHeader} onClick={() => setGrpCollapsed(c => ({ ...c, [grpKey]: !c[grpKey] }))}>
                                        ...
                                        <button style={{ ...S.quickBtn, fontSize: 9, padding: '1px 5px', marginLeft: 4 }}
                                          onClick={e => { e.stopPropagation(); setModalForm({ ...EMPTY_FORM, city_id: city.id, category: cat, group_name: grpName }); setModalMode('npc'); setShowModal(true); }}>+</button>
                                      </div>
                                      {!isGrpColl && grpNpcs.map(npc => {
                                        const isActive = selected === npc.id;
                                        return (
                                          <div key={npc.id} style={{ ...S.npcRowIndented, background: isActive ? 'rgba(184,137,42,0.09)' : undefined }}
                                            onClick={() => setSelected(isActive ? null : npc.id)}>
                                            ...
                                            <NpcButtons npc={npc} cityName={city.name} />
                                          </div>
                                        );
                                      })}
                                    </div>
                                ...
                                {ungrouped.map(npc => {
                                  const isActive = selected === npc.id;
                                  return (
                                    <div key={npc.id} style={{ ...S.npcRow, background: isActive ? 'rgba(184,137,42,0.09)' : undefined }}
                                      onClick={() => setSelected(isActive ? null : npc.id)}>
                                      <div style={{ ...S.npcDot, background: sc(npc.status) }} />
                                      <div style={S.npcMain}>
                                        <div style={S.npcName}>{npc.name}</div>
                                ...
                                <div style={{ padding: '3px 18px 4px' }}>
                                  <button style={{ ...S.btnGrey, fontSize: 9, padding: '2px 7px' }}
                                    onClick={() => { setNewGroupForm({ city_id: city.id, category: cat, name: '' }); setModalMode('group'); setShowModal(true); }}>
                                    + Add Group
                                  </button>
                                </div>
                              </>
          ...
          {npcs.map(npc => (
            <div key={npc.id} style={{ padding: '7px 10px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ fontSize: 9, fontFamily: "'Cinzel', serif", color: 'rgba(184,137,42,0.4)', letterSpacing: '0.06em' }}>{cities.find(c => c.id === npc.city_id)?.name}</div>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 5, marginLeft: 13 }}>
                {(npc.conditions || []).map(c => (
                  <div key={c} onClick={() => toggleNpcCondition(npc.id, c)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 3, background: `${condColor(c)}18`, border: `1px solid ${condColor(c)}55`, borderRadius: 20, padding: '2px 7px', fontFamily: "'Cinzel', serif", fontSize: 9, color: condColor(c), cursor: 'pointer', letterSpacing: '0.04em' }}>
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: condColor(c) }} />
                    {c} <span style={{ opacity: 0.6, marginLeft: 2 }}>✕</span>
                  </div>
                ))}
                <button onClick={e => setCondPicker({ entityId: npc.id, entityType: 'npc', anchorRect: e.currentTarget.getBoundingClientRect() })}
                  style={{ display: 'inline-flex', alignItems: 'center', background: 'rgba(184,137,42,0.08)', border: '1px solid rgba(184,137,42,0.25)', borderRadius: 20, padding: '2px 7px', fontFamily: "'Cinzel', serif", fontSize: 9, color: '#e8c040', cursor: 'pointer', letterSpacing: '0.04em' }}>
                  + Add
                </button>
              </div>
      ...
      {condPicker && (
        <div style={{ position: 'fixed', zIndex: 999999, width: 210, background: '#0a0805', border: '1px solid rgba(184,137,42,0.3)', borderRadius: 8, boxShadow: '0 16px 48px rgba(0,0,0,0.8)', overflow: 'hidden', top: Math.min((condPicker.anchorRect?.bottom || 200) + 4, window.innerHeight - 280), left: Math.min((condPicker.anchorRect?.left || 100), window.innerWidth - 220) }}
          ...
          <input autoFocus value={condSearch} onChange={e => setCondSearch(e.target.value)} placeholder="Search conditions…"
          ...
          <div style={{ maxHeight: 220, overflowY: 'auto', padding: '4px 0' }}>
            {CONDITIONS.filter(c => !condSearch || c.toLowerCase().includes(condSearch.toLowerCase())).map(cond => {
              const currentConds = condPicker.entityType === 'player'
                ...
                : (npcs.find(n => n.id === condPicker.entityId)?.conditions || []);
              ...
              return (
                <button key={cond} onClick={() => {
                  ...
                  else toggleNpcCondition(condPicker.entityId, cond);
      ...
      {selectedNpc && selectedCity && (
        <div style={S.drawer}>
          ...
          <div style={S.field}>
            <label style={S.label}>Name</label>
            <input style={S.fieldInput} value={selectedNpc.name} onChange={e => updateNpcField(selectedNpc.id, 'name', e.target.value)} />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ ...S.field, flex: 2 }}>
              <label style={S.label}>Role / Title</label>
              <input style={S.fieldInput} value={selectedNpc.role || ''} onChange={e => updateNpcField(selectedNpc.id, 'role', e.target.value)} placeholder="Role or title…" />
            </div>
            <div style={{ ...S.field, flex: 1 }}>
              <label style={S.label}>Status</label>
              <select style={S.fieldSelect} value={selectedNpc.status || 'Active'} onChange={e => updateNpcField(selectedNpc.id, 'status', e.target.value)}>
                {STATUS_OPTIONS.map(s => <option key={s} value={s} style={{ background: '#0e0c09' }}>{s}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ ...S.field, flex: 1 }}>
              <label style={S.label}>Category</label>
              <select style={S.fieldSelect} value={selectedNpc.category || 'Uncategorized'} onChange={e => updateNpcField(selectedNpc.id, 'category', e.target.value)}>
                {CATEGORY_OPTIONS.map(c => <option key={c} value={c} style={{ background: '#0e0c09' }}>{c}</option>)}
              </select>
            </div>
            <div style={{ ...S.field, flex: 1 }}>
              <label style={S.label}>Group</label>
              <input style={S.fieldInput} value={selectedNpc.group_name || ''} onChange={e => updateNpcField(selectedNpc.id, 'group_name', e.target.value)} placeholder="Group name or leave blank" />
            </div>
          </div>
          <div style={S.field}>
            <label style={S.label}>Faction</label>
            <input style={S.fieldInput} value={selectedNpc.faction || ''} onChange={e => updateNpcField(selectedNpc.id, 'faction', e.target.value)} placeholder="Faction or affiliation…" />
          </div>
          <div style={S.field}>
            <label style={S.label}>Notes</label>
            <textarea style={S.fieldTextarea} value={selectedNpc.notes || ''} onChange={e => updateNpcField(selectedNpc.id, 'notes', e.target.value)} placeholder="DM notes, relationships, plot hooks…" />
          </div>
          <div style={S.field}>
            <label style={S.label}>Conditions</label>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
              {(selectedNpc.conditions || []).map(c => (
                <div key={c} onClick={() => toggleNpcCondition(selectedNpc.id, c)}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 3, background: `${condColor(c)}18`, border: `1px solid ${condColor(c)}55`, borderRadius: 20, padding: '2px 7px', fontFamily: "'Cinzel', serif", fontSize: 9, color: condColor(c), cursor: 'pointer', letterSpacing: '0.04em' }}>
                  <div style={{ width: 5, height: 5, borderRadius: '50%', background: condColor(c) }} />
                  {c} <span style={{ opacity: 0.6, marginLeft: 2 }}>✕</span>
                </div>
              ))}
              <button onClick={e => setCondPicker({ entityId: selectedNpc.id, entityType: 'npc', anchorRect: e.currentTarget.getBoundingClientRect() })}
                style={{ display: 'inline-flex', alignItems: 'center', background: 'rgba(184,137,42,0.08)', border: '1px solid rgba(184,137,42,0.25)', borderRadius: 20, padding: '2px 7px', fontFamily: "'Cinzel', serif", fontSize: 9, color: '#e8c040', cursor: 'pointer', letterSpacing: '0.04em' }}>
                + Add
              </button>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              <button onClick={() => markMet(selectedNpc, selectedCity.name)}
              ...
              <button onClick={() => window.dispatchEvent(new CustomEvent('hercules:add_npc', { detail: { id: selectedNpc.id, name: selectedNpc.name, role: selectedNpc.role || '', conditions: selectedNpc.conditions || [], cityName: selectedCity.name } }))}
