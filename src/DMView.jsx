  ...
  const fetchMusic = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${SUPABASE_URL}/storage/v1/object/list/music`,
        {
          method: 'POST',
          headers: {
      ...
      const audioFiles = (Array.isArray(data) ? data : [])
        .filter(file => file.name && /\.(wav|mp3|ogg|flac|m4a)$/i.test(file.name))
        .map(file => ({
          title: file.name.replace(/\.(wav|mp3|ogg|flac|m4a)$/i, ''),
          file_path: file.name,
        }));
    ...
    <div>
      ...
      {currentTrack && (
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: 16, marginBottom: 16 }}>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: 11, color: COLORS.text, marginBottom: 10, letterSpacing: '0.04em' }}>
            ♪ {currentTrack.title}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filteredTracks.map(track => (
              <button
                ...
                style={{ textAlign: 'left', background: currentTrack?.file_path === track.file_path ? COLORS.magicBg : COLORS.card, border: `1px solid ${currentTrack?.file_path === track.file_path ? COLORS.magic : COLORS.border}`, borderRadius: 6, padding: '10px 12px', color: COLORS.text, cursor: 'pointer', fontFamily: 'Georgia, serif' }}
              >
        ...
        return (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button onClick={() => setShowArchive(!showArchive)} style={{ background: 'transparent', border: `1px solid ${COLORS.border}`, borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: 7, letterSpacing: '0.1em', textTransform: 'uppercase', color: COLORS.dim, fontFamily: "'Cinzel', serif" }}>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <button
                          onClick={session.archived ? (e) => unarchiveSession(e, session.session_id) : (e) => archiveSession(e, session.session_id)}
