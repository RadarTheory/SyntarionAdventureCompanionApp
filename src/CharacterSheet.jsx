...
// CHARACTER SHEET
// Props:
//   char     — full character object from buildChar()
//   onHome   — fn() back to Landing
//   onEdit   — fn() back to wizard (optional, for future)
// ═════════════════════════════════════════════════════════════════════════════
...
function Field({ label, value, dim }) {
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      }}>{value || <span style={{ color: COLORS.dim, fontStyle: 'italic' }}>—</span>}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 8, color: COLORS.dim, fontFamily: 'Georgia, serif', fontStyle: 'italic' }}>
...
function ActionRow({ action, bonus }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, color: bonus ? COLORS.magicText : COLORS.dim, fontFamily: 'Georgia, serif' }}>
        {bonus ? `+${bonus}` : '—'}
      </div>
    </div>
  );
...
// MAIN COMPONENT
// ═════════════════════════════════════════════════════════════════════════════
export default function CharacterSheet({ char, onHome }) {
  ...
  // ── ACTION SETS ──
  const cpActions = char.cp === 'magic' ? ACTIONS.magic : char.cp === 'tech' ? ACTIONS.tech : [];
  const allActions = [
    ...ACTIONS.universal,
    ...ACTIONS.combat,
    ...ACTIONS.social,
    ...
    switch (activeTab) {
      ...
      case 'identity': return (
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
            <div>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: isMobile ? 22 : 28, fontWeight: 700, color: COLORS.text, letterSpacing: '0.04em', lineHeight: 1.1 }}>
                {char.name || 'Unnamed'}
              </div>
              <div style={{ fontSize: 11, color: COLORS.muted, fontFamily: 'Georgia, serif', fontStyle: 'italic', marginTop: 4 }}>
                {raceDisplay}{cls ? ` · ${cls.name}` : ''}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            {[
              { label: 'Level', value: char.charLevel || 1 },
              { label: 'AP Current', value: char.apCurrent || 0 },
              { label: 'AP Total', value: char.apTotal || 0 },
            ].map(({ label, value }) => (
      ...
      case 'background': return (
        <div>
          ...
          {char.boonOrigin && (
            <p style={{ fontSize: 12, color: COLORS.textSub, fontFamily: 'Georgia, serif', fontStyle: 'italic', lineHeight: 1.75, marginBottom: 8 }}>
              {char.boonOrigin}.
            </p>
          )}
          {char.boonRole && (
            <p style={{ fontSize: 12, color: COLORS.textSub, fontFamily: 'Georgia, serif', fontStyle: 'italic', lineHeight: 1.75, marginBottom: 12 }}>
              {char.boonRole}.
            </p>
          )}
          {char.boonPersonality && (
            <div style={{ paddingTop: 12, borderTop: `1px solid ${COLORS.border}`, marginBottom: 20 }}>
              <p style={{ fontSize: 12, color: COLORS.text, fontFamily: 'Georgia, serif', lineHeight: 1.75, marginBottom: 8 }}>
                {char.boonPersonality}
              </p>
              {char.boonGrant && (
          ...
          {char.complication && (
            <>
              <SectionHead>Campaign Hook</SectionHead>
              <div style={{ background: COLORS.card, border: `1px solid ${COLORS.borderMid}`, borderRadius: 8, padding: '14px 16px', marginBottom: 20 }}>
                <p style={{ fontSize: 12, color: COLORS.textSub, fontFamily: 'Georgia, serif', fontStyle: 'italic', lineHeight: 1.7, margin: 0 }}>
                  {char.complication}
                </p>
              </div>
            </>
          ...
          {char.notes && (
            <>
              ...
              <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '14px 16px' }}>
                <p style={{ fontSize: 12, color: COLORS.textSub, fontFamily: 'Georgia, serif', lineHeight: 1.75, margin: 0 }}>
                  {char.notes}
                </p>
              </div>
            </>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 10, color: COLORS.muted, fontFamily: 'Georgia, serif' }}>
              Total: <span style={{ color: COLORS.text, fontWeight: 700, fontFamily: "'Cinzel', serif" }}>
                {ALL_STATS.reduce((sum, s) => {
                  const base = char.stats?.[s.key] || 8;
                  const item = char.itemBonuses?.[s.key] || 0;
                  const cond = char.conditions?.[s.key] || 0;
                  const occ  = char.occurrences?.[s.key] || 0;
                  return sum + base + item + cond + occ;
                }, 0)}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12, marginBottom: 24 }}>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {magicStats.map(s => <StatDisplay key={s.key} statDef={s} char={char} />)}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {techStats.map(s => <StatDisplay key={s.key} statDef={s} char={char} />)}
              </div>
            </div>
          </div>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '12px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
              {[['i', 'Item bonus'], ['c', 'Condition modifier'], ['o', 'Occurrence (permanent)']].map(([k, v]) => (
                <div key={k} style={{ fontSize: 10, color: COLORS.muted, fontFamily: 'Georgia, serif' }}>
                  <span style={{ color: COLORS.text, fontWeight: 700 }}>{k}</span> — {v}
                </div>
              ))}
            </div>
          </div>
      ...
      case 'actions': return (
        <div>
          {Object.entries(ACTIONS).map(([category, actions]) => {
            // Filter tech/magic specific actions by class path
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {actions.map(action => (
                    <ActionRow
                      key={action}
                      action={action}
                      bonus={char.actionBonuses?.[action]}
                    />
                  ))}
                </div>
              </div>
      ...
      case 'inventory': return (
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            {APPAREL_SLOTS.map(slot => (
              <EquipRow key={slot} slot={slot} value={char.apparel?.[slot]} />
            ))}
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {WEAPON_SLOTS.map(slot => (
              <EquipRow key={slot} slot={slot} value={char.weapons?.[slot]} />
            ))}
          </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {ACCESSORY_SLOTS.map(slot => (
              <EquipRow key={slot} slot={slot} value={char.accessories?.[slot]} />
            ))}
          </div>
          ...
          {char.inventory?.pack && (
            <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: '12px 14px' }}>
              ...
              <p style={{ fontSize: 12, color: COLORS.textSub, fontFamily: 'Georgia, serif', lineHeight: 1.7, margin: 0 }}>
                {char.inventory.pack}
              </p>
            </div>
          )}
