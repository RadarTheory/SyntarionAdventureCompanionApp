...
import FubinLauncher from './FubinLauncher';
...
export default function App() {
  ...
  if (view === 'bag') {
    return <LotjarrsBag onHome={() => setView('landing')} onLaunchGame={id => setView(id)} />;
  }

  if (view === 'driftstone') {
    return <PlayDriftstone onHome={() => setView('bag')} />;
  }

  if (view === 'fubin') {
    return <FubinLauncher onHome={() => setView('bag')} />;
  }
...
function LoginScreen() {
  ...
  const inputStyle = {
    ...
    border: '1px solid rgba(26,23,20,0.25)', borderRadius: 6,
    fontFamily: 'Georgia, serif', fontSize: 13, color: '#1a1714',
    outline: 'none', marginBottom: 10,
  };

  const btnStyle = {
    width: '100%', background: '#1a1714', border: 'none', borderRadius: 6,
    padding: '14px 32px', cursor: 'pointer', fontFamily: "'Cinzel', serif",
    fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase',
    color: '#f0eeeb', fontWeight: 700, marginBottom: 10,
  };

  const ghostBtnStyle = {
    ...btnStyle, background: 'transparent',
    border: '1px solid rgba(26,23,20,0.25)', color: '#1a1714',
  };

  return (
    <div style={{ minHeight: '100vh', background: '#f0eeeb', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'Georgia, serif', padding: 24 }}>
      ...
      <div style={{ width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column' }}>
        ...
        <input style={inputStyle} type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleEmailAuth(); }} />
        ...
        <button onClick={handleEmailAuth} style={btnStyle}>{isSignUp ? 'Create Account' : 'Sign In'}</button>

        <div style={{ textAlign: 'center', fontSize: 12, color: 'rgba(26,23,20,0.5)', marginBottom: 20, cursor: 'pointer' }} onClick={() => setIsSignUp(c => !c)}>
