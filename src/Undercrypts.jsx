...
const TOWN_LOCATIONS = [
  { id: 'inn', name: 'The Low Lantern Inn', lead: 'Rest, rumors, and paid cowardice.', action: 'Rest', reply: 'The innkeeper marks your name on a slate. If you return, she says, the first drink is not free.' },
  { id: 'market', name: 'Blackstep Market', lead: 'Scrap charms, canal knives, and old civic keys.', action: 'Browse', reply: 'A trader shows you chalk, oil, and a lockpick bent into the shape of a prayer.' },
  { id: 'archive', name: 'Civic Reliquary', lead: 'Town records sit beside recovered crypt tablets.', action: 'Ask', reply: 'The reliquary notes a sealed municipal stair beneath the old charter house. Not a tomb. Worse: infrastructure.' },
  { id: 'engineer', name: 'Veinrunner Workshed', lead: 'Ingenium lamps flicker against old ward-glass.', action: 'Check gear', reply: 'The engineer taps your weapon and mutters that magic makes cowards of tolerances.' },
  { id: 'gate', name: 'The Undercrypt Gate', lead: 'A civic door descends under Ashendell, newly breathing cold air.', action: 'Descend', reply: 'The gate opens with a dry metallic cough. Your crawl begins below the lower town.' },
];
...
export default function Undercrypts({ onExit }) {
  ...
  const affinity = useMemo(() => affinityFor(race, origin), [race, origin]);
  ...
  if (screen === 'menu') return <Menu race={race} setRace={setRace} bloodline={bloodline} setBloodline={setBloodline} bloodlines={bloodlines} origin={origin} setOrigin={setOrigin} build={build} unlocks={unlocks} affinity={affinity} onExit={onExit} onStart={() => setScreen('town')} />;
  if (screen === 'town') return <TownHub race={race} bloodline={bloodline} origin={origin} build={build} unlocks={unlocks} affinity={affinity} townLog={townLog} onExit={onExit} onEdit={() => setScreen('menu')} onVisit={(loc) => { if (loc.id === 'gate') startRun(); setTownLog(log => [loc.reply, ...log].slice(0, 5)); }} />;
...
function TownHub({ race, bloodline, origin, build, unlocks, affinity, townLog, onExit, onEdit, onVisit }) {
  const selectedOrigin = ORIGINS.find(o => o.id === origin) || ORIGINS[0];
  return <div className="uc-shell uc-town"><style>{styles}</style><button className="uc-back" onClick={onExit}>Back to Bag</button><header className="uc-town-head"><div><div className="uc-kicker">Ashendell single-town prototype</div><h1>Ashendell Undercrypt</h1><p>A town hub above the old municipal dark.</p></div><button onClick={onEdit}>Edit Wanderer</button></header><main className="uc-town-grid"><section className="uc-map-card"><div className="uc-town-map"><img src="/Maps/Ashendell.png" alt="Ashendell map" />{TOWN_LOCATIONS.map((loc, i) => <button key={loc.id} className={`loc loc-${i}`} onClick={() => onVisit(loc)}><b>{loc.name}</b><span>{loc.lead}</span><em>{loc.action}</em></button>)}</div></section><aside className="uc-side"><div className="uc-card"><h2>Wanderer</h2><p>{raceLabel(race, bloodline)}</p><p>{selectedOrigin.name}</p><p>{affinityLabel(affinity)} ({affinity})</p><p>{build.name}</p></div><div className="uc-card"><h2>Town Record</h2><div className="uc-log">{townLog.map((l, i) => <p key={i}>{l}</p>)}</div></div><div className="uc-card"><h2>Unlocked Town Pool</h2><div className="uc-unlocks small">{ITEMS.filter(i => unlocks.includes(i.id)).map(i => <span key={i.id} className="on">{i.name}</span>)}</div></div></aside></main></div>;
}

function Menu({ race, setRace, bloodline, setBloodline, bloodlines, origin, setOrigin, build, unlocks, affinity, onExit, onStart }) {
  return <div className="uc-shell"><style>{styles}</style><button className="uc-back" onClick={onExit}>Back to Bag</button><section className="uc-hero"><div className="uc-kicker">Lotjarr's Bag of Games</div><h1>The Undercrypts of Soteria</h1><p>Descend, survive, return changed.</p></section><main className="uc-menu"><div className="uc-card"><h2>Create Wanderer</h2><select value={race} onChange={e => setRace(e.target.value)}>{RACES.map(r => <option key={r.id} value={r.id}>{r.name} - {r.sub}</option>)}</select>{race === 'pamorph' && <select value={bloodline} onChange={e => setBloodline(e.target.value)}>{bloodlines.map(b => <option key={b.id} value={b.id}>{b.name} - {b.sub}</option>)}</select>}<select value={origin} onChange={e => setOrigin(e.target.value)}>{ORIGINS.map(o => <option key={o.id} value={o.id}>{o.name} - {o.note}</option>)}</select><div className="uc-stance"><b>{build.name}</b><span>{build.note}</span><em>Style: {build.style} / Reach: {build.reach} / Damage: {build.damage}</em><em>{affinityLabel(affinity)}: {affinity}</em></div><button className="uc-primary" onClick={onStart}>Enter Ashendell</button></div><div className="uc-card"><h2>Unlocked Pool</h2><div className="uc-unlocks">{ITEMS.map(i => <span key={i.id} className={unlocks.includes(i.id) ? 'on' : ''}>{i.name}</span>)}</div><p>Held inventory resets at the end of each crawl. Unlocked items stay unlocked and enter future room reward pools.</p></div></main></div>;
}
...
.uc-town-head { max-width:1380px; margin:20px auto 16px; display:flex; justify-content:space-between; gap:18px; align-items:end; } .uc-town-head button { width:auto; min-width:150px; border:1px solid rgba(214,177,84,.42); border-radius:8px; background:linear-gradient(#3b2c12,#120c06); color:#f0d992; min-height:38px; padding:0 12px; font-family:Cinzel,serif; font-size:10px; letter-spacing:.12em; text-transform:uppercase; cursor:pointer; } .uc-town-grid { max-width:1380px; margin:0 auto; display:grid; grid-template-columns:minmax(720px,1fr) 330px; gap:18px; } .uc-map-card { border:1px solid rgba(214,177,84,.24); border-radius:18px; padding:14px; background:linear-gradient(145deg,#171006,#070504); box-shadow:0 30px 90px rgba(0,0,0,.55); } .uc-town-map { position:relative; min-height:620px; overflow:hidden; border-radius:12px; background:#080503; isolation:isolate; } .uc-town-map::after { content:""; position:absolute; inset:0; z-index:1; background:radial-gradient(circle at 50% 46%, rgba(3,2,1,.06), rgba(3,2,1,.5) 72%, rgba(3,2,1,.78)), linear-gradient(rgba(5,3,2,.08),rgba(5,3,2,.22)); pointer-events:none; } .uc-town-map img { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; object-position:center; opacity:.82; filter:saturate(.92) contrast(1.05) brightness(.86); } .loc { position:absolute; z-index:2; width:220px; min-height:112px; text-align:left; border:1px solid rgba(214,177,84,.36); border-radius:12px; background:linear-gradient(145deg,rgba(30,24,15,.92),rgba(7,6,4,.92)); color:#eadfca; padding:12px; cursor:pointer; box-shadow:0 18px 48px rgba(0,0,0,.44); } .loc:hover { border-color:rgba(125,211,201,.75); transform:translateY(-2px); } .loc b,.loc span,.loc em { display:block; } .loc b { color:#f0d992; font-family:Cinzel,serif; font-size:12px; letter-spacing:.08em; } .loc span { margin-top:8px; color:rgba(234,223,202,.7); font-size:12px; } .loc em { margin-top:10px; color:#84d5cc; font-size:10px; text-transform:uppercase; letter-spacing:.12em; } .loc-0{left:8%;top:14%}.loc-1{left:38%;top:8%}.loc-2{right:8%;top:20%}.loc-3{left:20%;bottom:14%}.loc-4{right:16%;bottom:12%;border-color:rgba(214,177,84,.72)}
@media (max-width:1080px){ .uc-menu,.uc-play,.uc-town-grid{grid-template-columns:1fr}.uc-top,.uc-town-head{flex-direction:column;text-align:center;align-items:center}.uc-vitals{width:min(100%,360px)}.uc-town-map{min-height:760px}.loc{position:relative;left:auto!important;right:auto!important;top:auto!important;bottom:auto!important;width:auto;margin:12px;} }
`;
